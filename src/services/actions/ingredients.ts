...
export const INCREMENT_ID_INGRIDIENT = 'INCREMENT_ID_INGRIDIENT';
export const DECREMENT_ID_INGRIDIENT = 'INCREMENT_ID_DECREMENT';
...
export const getDataIngridietn = (url: string) => (dispatch: any) => {
  fetch(url)
    ...
    .then((response) => {
      if (response.success) {
        const objIngridients: any = {};
        response.data.forEach(({ _id }: any) => (objIngridients[_id] = 0));
        dispatch({
          type: SET_INGRIDIENT_BURGER,
          items: response.data,
          objKey: objIngridients,
        });
        dispatch({
